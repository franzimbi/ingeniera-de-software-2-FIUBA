...
describe 'Pruebas web personales-Fran' do
  ...
  it 'cotizar un envio de 6 cuadrados' do
    ...
    get "/cotizacion/#{id}/checkout"
    expected_response = { "envio" => "CCCCCC", "volumen-total" => 6, "costo" => 9.0 }.to_json
    expect(last_response.body).to eq(expected_response)
  end
  ...
  it 'envio con mas de 11 litros es demasiado grande' do
    get '/cotizacion'
    expect(last_response.status).to eq 200
    response_body = JSON.parse(last_response.body)
    id = response_body['id']
  ...
  it 'e6: envio con volumen mayor a 11 es demasiado grande' do
    resultado = enviar(['T','T','T','T'])
    expect(resultado.status).to eq 400
    expect(resultado.descripcion).to eq 'ENVIO_DEMASIADO_GRANDE'
  end

  it 'Extension1: cotizar un paquete rectangular' do
    get '/cotizacion'
    expect(last_response.status).to eq 200
    response_body = JSON.parse(last_response.body)
    id = response_body['id']
    post "/cotizacion/#{id}", { paquete: "E" }.to_json, { "CONTENT_TYPE" => "application/json" }
    expect(last_response.status).to eq 200
    get "/cotizacion/#{id}/checkout"
    expected_response = { "envio" => "E", "volumen-total" => 4, "costo" => 2.2 }.to_json
    expect(last_response.body).to eq(expected_response)
  end

  it 'Extension2: cotizar dos paquetes rectangulares' do
    get '/cotizacion'
    expect(last_response.status).to eq 200
    response_body = JSON.parse(last_response.body)
    id = response_body['id']
    ...
    expect(last_response.status).to eq 200
    get "/cotizacion/#{id}/checkout"
    expected_response = { "envio" => "EE", "volumen-total" => 8, "costo" => 4.0 }.to_json
    expect(last_response.body).to eq(expected_response)
  end

  it 'Extension3: cotizar un paquete rectangular con uno redondo no puede ser enviado' do
    get '/cotizacion'
    expect(last_response.status).to eq 200
    response_body = JSON.parse(last_response.body)
    id = response_body['id']
  ...
  it 'Extension4: envio con 11 litros no es demasiado grande' do
    get '/cotizacion'
    expect(last_response.status).to eq 200
    response_body = JSON.parse(last_response.body)
    id = response_body['id']
    ...
    #
    get "/cotizacion/#{id}/checkout"
    expected_response = { "envio" => "EECCC", "volumen-total" => 11, "costo" => 10.0 }.to_json
    expect(last_response.body).to eq(expected_response)
  end

  it 'Extension5: envio con mas de 11 litros es demasiado grande' do
    get '/cotizacion'
    expect(last_response.status).to eq 200
    response_body = JSON.parse(last_response.body)
    id = response_body['id']
  ...
  it 'Extension6: cotizar un envio con un paquete rectangular y un triangular' do
    get '/cotizacion'
    expect(last_response.status).to eq 200
    response_body = JSON.parse(last_response.body)
    id = response_body['id']
    ...
    get "/cotizacion/#{id}/checkout"
    expect(last_response.status).to eq 200
    expected_response = { "envio" => "TE", "volumen-total" => 7, "costo" => 5.0 }.to_json
    expect(last_response.body).to eq(expected_response)
  end

  it 'ex1: envio de volumen 11 es valido' do
    resultado = enviar(['R','R','R','R','R','C'])
    expect(resultado.status).to eq 200
  end

  it 'ex2: envio de volumen 12 NO es valido' do
    resultado = enviar(['R','R','R','R','R','C', 'C'])
    expect(resultado.status).to eq 400
  end

  it 'ex3: 2 paquetes rectangulares cuestan 4' do
    resultado = enviar(['E', 'E'])
    expect(resultado.status).to eq 200
    expect(resultado.costo).to eq 4
  end

  it 'ex4: paquetes rectangulares no pueden enviarse con redondos' do
    resultado = enviar(['E', 'R'])
    expect(resultado.status).to eq 400
  end
